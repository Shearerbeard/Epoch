...
use crate::{
    decider::{DeciderWithContext, Evolver, InitializeableState},
    repository::{
        self, event::VersionedRepositoryError, in_memory::state, state::VersionedStateRepository,
        RepositoryVersion,
    },
};
...
where
    ...
    <Self::Ev as Evolver>::State: Send + Sync + Debug + InitializeableState,
    <<Self::Ev as Evolver>::State as InitializeableState>::Creator: Debug + Send + Sync,
{
    type Ev: Evolver + Send + Sync;

    async fn load<'a, Err>(
        state_creator: <<Self::Ev as Evolver>::State as InitializeableState>::Creator,
        ...
        Err: Debug + Send + Sync,
    {
        Ok(event_repository
            ...
            .0
            .iter()
            .fold(<Self::Ev as Evolver>::init(state_creator), Self::Ev::evolve))
    }
...
where
    ...
    <<Self::Decide as Evolver>::State as InitializeableState>::Creator: Debug + Send + Sync,
{
    ...
    async fn execute<'a, RepoErr, StreamId>(
        state_creator: <<Self::Decide as Evolver>::State as InitializeableState>::Creator,
            ...
            + StreamIdFromEvent<<<Self as LoadDecideAppend>::Decide as Evolver>::Evt>,
    {
        ...
        let mut state = <Self::Decide as Evolver>::init(state_creator);
